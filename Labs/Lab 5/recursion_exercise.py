...
def max_length(obj: Union[list, object]) -> int:
    ...
    4
    >>> max_length([[1, 2, [3, 4]], 4, [5, 6]])
    3
    >>> max_length([[1, 2, [1, 2, 3, 4, 5]], 4, 5, [5, 6]])
    5
    """
    if isinstance(obj, int):
        return 0
    elif not any([isinstance(x, list) for x in obj]):
        return len(obj)
    else:
        a = max([max(max_length(x), len(x)) if isinstance(x, list)
                 else 0 for x in obj])
        return max(a, len(obj))
...
def count_above(obj: Union[list, int], n: int) -> int:
    ...
    >>> count_above([17, 18, [19, 20]], 18)
    2
    >>> count_above([2, 3, [1, 3, 5, [3, 5, [5 ,7]]], 6], 2)
    8
    """
    if isinstance(obj, list):
