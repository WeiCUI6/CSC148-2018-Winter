...
def num_lists(obj: Union[Any, List]) -> int:
    ...
    if not isinstance(obj, list):
        ...
        return 1 + sum([num_lists(c) for c in obj])
...
class Tree:
    ...
    def sum_values(self):
        ...
        if not self.children:
            ...
            return sum([c.sum_values() for c in self.children] + [self.value])
    ...
    def max_value(self):
        ...
        if not self.children:
            ...
            return max([c.max_value() for c in self.children] + [self.value])
    ...
    def sum_values_above_n(self, n):
        ...
        if not self.children:
            ...
            if self.value > n:
                return sum([c.sum_values_above_n(n) for c in self.children] + [self.value])
            else:
                return sum([c.sum_values_above_n(n) for c in self.children])
    ...
    def longest_path(self):
        ...
        if not self.children:
            ...
            paths = [c.longest_path() for c in self.children]
            paths = [[self.value] + x for x in paths]
            lens = [len(x) for x in paths]
            lens = lens[::-1]
            return paths[lens.index(max(lens))]
    ...
    def sum_at_depth(self, depth):
        ...
        if depth < 0:
            ...
            return sum([c.sum_at_depth(depth - 1) for c in self.children])
